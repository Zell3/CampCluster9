...
<head>
    ...
    <body>
        ...
        <div class="tools-flex-box">
            ...
            <div class="item">
                <div class="searchBar">
                    <input type="text" id="searchInput" placeholder="Search" class="inputSearch"
                        style="margin-right: 10px;">
                    <button onclick="search()" class="btn-search-bg"><svg class="iconColor"
                ...
                <form>
                    <div class="container-grid">
                        ...
                        <div class="type">
                            ...
                            <br>
                            <input type="checkbox" id="jobApplication" name="type" value="พนักงานทั่วไป">
                            <label for="jobApplication">พนักงาน</label>
                            <br>
                            <input type="checkbox" id="internship" name="type" value="สหกิจศึกษา">
                            <label for="internship">สหกิจ</label>
                        </div>
                        <br>
                        <div>
                            <label class="statusEmail">สถานะอีเมล</label>
                            <br>
                            <input type="checkbox" id="waiting" name="emailStatus" value="wait">
                            <label for="waiting">รอตอบกลับ</label>
                            <br>
                            <input type="checkbox" id="responded" name="emailStatus" value="done">
            ...
            <table class="table table-striped table-sm">
                ...
                <tbody class="show-data">
                    ...
                    <tr class="searchable">
                        ...
                        <td><?php echo $applicant->bdu_name; ?> <?php echo $applicant->bdu_lastname; ?></td>
                        <td><?php echo $applicant->bdu_working; ?></td>
                        <td><?php echo $applicant->role->ro_name; ?></td>
                        <td><a href="{{ route('form.primary', ['id' => $applicant->bdu_id]) }}"><button
                                    class="btn-link">ฟอร์มเบื้องต้น</button></a></td>
                        <td><a href="{{ route('showAdditionalData', ['id' => $applicant->bdu_id]) }}"><button
                                    class="btn-link">ฟอร์มเพิ่มเติม</button></a></td>
                        <td>
                            <?php
                            // เช็คค่า bdu_recieve_email ว่าเป็น 0 หรือ 1
                            $status = $applicant->bdu_receive_email == 1 ? 'Done' : 'Wait';
                            // กำหนดคลาส CSS สำหรับสถานะ
                            $class = $applicant->bdu_receive_email == 1 ? 'done-status' : 'wait-status';
                            ?>
        ...
        <script>
            ...
            function filterTableRows(startDate, endDate, type, statuses, emailStatuses, position) {
                ...
                for (let i = 0; i < tableBody.rows.length; i++) {
                    ...
                    const matchesDate = applicantDate >= new Date(startDate).getTime() && applicantDate <= new Date(endDate)
                        .getTime();
                    const matchesType = type.length === 0 || type.includes(applicantType);
                    const matchesEmailStatus = emailStatuses.length === 0 || emailStatuses.includes(applicantStatus.toLowerCase());
                    ...
                    if (matchesDate && matchesType && matchesEmailStatus && matchesPosition) {
                        row.style.display = '';
                    } else {
                        row.style.display = 'none';
                    }
                    console.log(matchesEmailStatus);
