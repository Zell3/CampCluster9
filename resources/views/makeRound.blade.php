@extends('dashboard')
@section('title', 'รอบสมัคร')
@section('content')

    <link rel="stylesheet" href="{{ asset('css/makeRound.css') }}">

    <div class="flexbox-make-round">
        <div class="item">
            <div class="content">
                <form>
                    <div>
                        <label for="title">ชื่อเรื่อง</label>
                        <input class="ti" type="text" name="title" required><br>
                    </div>
                    <div>
                        <label for="location">สถานที่</label>
                        <input class="lo" type="text" name="location" required><br>
                    </div>
                    <div>
                        <label for="position">ตำแหน่ง</label><br>
                        <input class="checkbox" type="checkbox" name="SA">
                        <label for="SA">System Analyst</label><br>

                        <input class="checkbox" type="checkbox" name="Tester">
                        <label for="Tester">Tester</label><br>

                        <input class="checkbox" type="checkbox" name="Programmer">
                        <label for="Programmer">Programmer</label><br>

                        <input class="checkbox" type="checkbox" name="BA">
                        <label for="BA">Business Analyst</label><br>
                    </div>

                    <div>
                        <label for="Applicant_type">ประเภทผู้สมัคร</label><br>
                        <input class="checkbox" type="checkbox" name="work">
                        <label for="work">ทำงาน</label><br>

                        <input class="checkbox" type="checkbox" name="Apprentice">
                        <label for="Apprentice">ฝึกงาน</label><br>
                    </div>
                    <div>
                        <label for="Start_date">วันเริ่ม</label>
                        <input class="date_s" type="date" name="Start_date" required><br>
                        <label for="End_date">วันสิ้นสุด</label>
                        <input class="date_e" type="date" name="End_date" required><br>
                    </div>
                    <div>
                        <label for="comment">Comment</label><br>
                        <textarea class="comment" name="comment" cols="30" rows="5"></textarea>
                    </div>
                </form>
            </div>
        </div>

        <div class="item">
            <div class="content">
                <div class="box2">
                    <img src="data:image/png;base64,"
                        alt="QR"><br>

                    <img src="data:image/png;base64,"
                        alt="QR">
                </div>
            </div>
            <input class="submit" type="submit" value="บันทึก">
        </div>

    </div>


@endsection
